...
#define CSVFILE_S3RPS_EXTRA_COMMAS  14 // extra columns when --s3-rps is enabled
...
/*s3r*/	(ARG_S3RPS_LONG, bpo::bool_switch(&this->showS3RPS),
			"Show S3 requests per second (RPS) in phase results, broken down by HTTP method: "
			"GET, PUT, HEAD, POST, DELETE, LIST. Also included in CSV/JSON output when enabled.")
...
void ProgArgs::printHelpS3()
{
	...
	argsS3FrequentDescription.add_options()
		...
		(ARG_S3RPS_LONG, bpo::bool_switch(&this->showS3RPS),
			"Show S3 requests per second (RPS) broken down by HTTP method: GET, PUT, HEAD, POST, "
			"DELETE, LIST.")
	;
...
void ProgArgs::checkCSVFileCompatibility()
{
	...
	int expectedCommas = CSVFILE_EXPECTED_COMMAS;
	if(showS3RPS)
		expectedCommas += CSVFILE_S3RPS_EXTRA_COMMAS;
	...
	if(numCommas != expectedCommas)
		throw ProgException("CSV output file exists. Trying to append, but column compatibility "
			...
			" (or with/without \"--" ARG_S3RPS_LONG "\")? "
			"Found commas: " + std::to_string(numCommas) + "; "
			"Expected commas: " + std::to_string(expectedCommas) + "; "
			"File: " + resFilePathCSV);
}
